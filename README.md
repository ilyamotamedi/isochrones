...
| `VITE_FIREBASE_*` | Analytics only. Omit them and the app works, minus analytics. |
...
Off, and it will stay off until someone decides it should be on.

`src/analytics/` is wired up: events fire from the app, the SDK is lazy-loaded
on an idle callback, and there is a consent gate in front of all of it. That
gate **fails closed** — the stored state starts as `unknown`, `unknown` is not
consent, and there is no UI yet to turn it into `granted`. So on a fresh visit
nothing is initialised and nothing is sent.

That is deliberate rather than unfinished. Analytics brings a consent
obligation in the EU and UK that this app does not currently have, because it
sets no cookies and phones nobody but Mapbox. Adding a consent banner is a
product decision, not a plumbing one.

What it costs while switched off, measured on the production build:

| | Raw | Gzipped |
| --- | --- | --- |
| Main bundle, before | 2,298.74 kB | 657.40 kB |
| Main bundle, after | 2,302.53 kB | 658.90 kB |
| **Eager cost** | **+3.79 kB** | **+1.50 kB** |
| Firebase chunks, lazy | 48.14 kB | 16.23 kB |

The three Firebase chunks are split out and carry no `modulepreload` link, so
they are fetched only if the gates pass. A visitor who has not consented — that
is, currently everyone — downloads the 1.5 kB and nothing else.

Events are a closed union in
[`src/analytics/events.ts`](src/analytics/events.ts); call sites cannot pass
free-form parameters. **No coordinates, addresses or search text are sent**,
and a unit test fails if a location-shaped key is added to the allow-list.
...
```
src/
  analytics/  Event vocabulary, consent gate, lazy Firebase init
