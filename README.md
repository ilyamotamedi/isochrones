...
on the token's page, under *URL restrictions*, add your deployed origin.
Mapbox will then reject requests carrying that token from anywhere else.

> [!WARNING]
> **Mapbox URL restrictions do not support wildcard characters.** An entry of
> `https://isochrones.example.com/*` is not a pattern — write the origin
> plainly, as `https://isochrones.example.com`.
>
> Restrictions also **authorise subdomains automatically**: `example.com`
> allows `www.example.com` and everything below it. That is convenient for
> your own domain and a trap on a shared one — see the Firebase Hosting note
> under [Deployment](#deployment).

Two further limitations from Mapbox's own docs, both of which produce a `403`
and a blank map rather than an obvious error:

- **Referrer policy.** Restrictions work by reading the `Referer` header, so a
  page served with `no-referrer` or `same-origin` will be rejected by its own
  token. The `Referrer-Policy` in `firebase.json` is deliberately
  `strict-origin-when-cross-origin`, which sends the origin and no path.
  Tightening it further will break the map.
- **Privacy blockers.** Brave Shields, Ghostery and similar extensions strip
  the referrer, which has the same effect. This is Mapbox's documented
  behaviour and there is nothing the app can do about it.

Requests from `localhost` are blocked too, so keep a second, unrestricted
token for local development rather than adding `localhost` to the live one.
...
Firebase Hosting, project `isochrones-4f3fa`. By hand:
...
Or push to `main` and let [`deploy.yml`](.github/workflows/deploy.yml) do it.
Pull requests get a preview channel that expires after seven days. Both paths
build from the same job, so a preview cannot be built differently from what
ships, and both are gated on `npm run typecheck` and `npm test`.

The Playwright harness is deliberately **not** in CI — it needs a running dev
server and takes three to four minutes. It stays a local pre-release step.

### Repository secrets

| Secret | Needed for |
| --- | --- |
| `VITE_MAPBOX_TOKEN` | The build. Without it the deployed app shows the setup notice. |
| `FIREBASE_SERVICE_ACCOUNT` | The deploy. Generated by `firebase init hosting:github`. |
| `VITE_GA_MEASUREMENT_ID` | Analytics only. Omit it and the app works, minus analytics and minus the consent banner. |

> [!CAUTION]
> **Preview channels and the Mapbox token restriction do not get along.**
>
> Hosting serves the live site on two domains — `isochrones-4f3fa.web.app` and
> `isochrones-4f3fa.firebaseapp.com` — so both need their own entry. Preview
> channels are worse: they get a generated hostname such as
> `isochrones-4f3fa--pr-12-a1b2c3.web.app`, which is a *sibling* of the live
> subdomain rather than a child of it, so the live entry does not cover it, and
> the names cannot be known in advance.
>
> Adding plain `web.app` would cover them — and every other Firebase site on
> the internet, which makes the restriction close to worthless. The honest
> options are a second, separately restricted token for previews, or accepting
> that previews render the UI over a blank map.

### Firestore

Scaffolded but unused. `firestore.rules` denies everything, and nothing in
`src/` imports the Firestore SDK — the caching design is still open. The
emulators are configured if you want to experiment:
...
Google Analytics 4, in **consent mode advanced**, installed as a raw Google tag
rather than through the Firebase SDK.

Set `VITE_GA_MEASUREMENT_ID` and it runs. Leave it blank and nothing happens at
all — no tag, and no consent banner either, because asking permission for
something that cannot occur teaches people that banners are noise.

#### The one thing that must not go wrong

The share URL carries the user's location in the clear — `lng` and `lat` to
five decimal places, which is about a metre, plus the reverse-geocoded street
address. GA attaches `page_location` to every event by reading
`document.location` itself, so connecting GA to this app naively would send
somebody's home address to Google on every interaction. The closed event union
does not help: that governs parameters, and this arrives through page context.

Three layers stand in the way, plus one setting:

1. [`sanitiseLocation`](src/analytics/location.ts) returns origin and path and
   nothing else. Total, not selective — an allow-list would need updating every
   time a parameter is added, and the cost of forgetting is somebody's address
   in an analytics pipeline.
2. It is applied at `config` time and re-applied via `gtag('set', …)` on every
   URL rewrite, so a page view fired by Enhanced Measurement still carries the
   clean value.
3. [`safePageLocation`](src/analytics/gtag.ts) re-checks the outgoing value
   against `isForbiddenParamKey` — the same guard the event dispatcher uses —
   and throws in development if anything got through. It is unreachable today,
   which is exactly why it is tested by mocking the sanitiser into a
   pass-through.
4. `Referrer-Policy: strict-origin-when-cross-origin` in `firebase.json` means
   the `Referer` header on requests to Google carries the origin only, never
   the query string.

> [!IMPORTANT]
> Turn **off** GA4 → Admin → Data streams → Enhanced measurement → "Page
> changes based on browser history events". Layer 2 makes it safe, but this app
> calls `replaceState` on every query and there is no reason to accept a page
> view per keystroke-settled interaction in the first place.

#### The mode switch

One constant in [`src/config.ts`](src/config.ts):

| `ANALYTICS_MODE` | The tag loads | A decliner sends |
| --- | --- | --- |
| `advanced` *(current)* | immediately, for everyone | cookieless pings |
| `basic` | only after an explicit yes | nothing at all |
| `off` | never | nothing at all |

The banner, the stored decision and the event vocabulary are identical in all
three, so changing your mind is a one-line edit rather than a refactor.

#### Why not `firebase/analytics`

It was tried first. `@firebase/analytics` calls `installations.getId()`
unconditionally on start — read in the installed source at v0.10.25, not
inferred from the docs — which writes a persistent Firebase Installation ID to
IndexedDB and registers it with Google. **For people who declined.** Consent
mode governs gtag's cookies and has nothing to say about that identifier.

Installing the tag by hand costs one file of plumbing and removes the
`firebase` dependency from the client entirely.

#### What it costs

Measured on the production build, `vite build` reporting:

| | Raw | Gzipped |
| --- | --- | --- |
| Main bundle, no analytics at all | 2,298.74 kB | 657.40 kB |
| Main bundle, via the Firebase SDK | 2,302.53 kB | 658.90 kB |
| Main bundle, now | 2,303.52 kB | 659.10 kB |
| CSS, before → now | 61.51 → 62.53 kB | 8.74 → 8.95 kB |
| Firebase lazy chunks | **gone** | **gone** |
| `gtag.js`, from googletagmanager | 430.28 kB | **152.23 kB** |

> [!WARNING]
> Advanced mode means **every visitor downloads 152 kB of `gtag.js`**, not just
> the ones who said yes. That is the deal, and it is the single largest cost of
> this design. Switch `ANALYTICS_MODE` to `basic` if you would rather the
> refusers paid nothing.

Two other honest caveats:

- **Behavioural modelling will not activate** until the property sees 1,000
  events a day with `analytics_storage='denied'` for seven days *and* 1,000
  daily users with it granted. Until then advanced mode buys aggregate counts
  from decliners, not gap-filling.
- **A cookieless ping is still a transfer to Google.** No storage is written,
  which is the legal hinge, but a request carrying IP and user agent does leave
  the browser for people who declined.

#### The banner

Bottom-centre, non-modal, two buttons of **equal visual weight** — the EDPB's
position is that refusing must be no harder than accepting, and it happens to
be the honest design because we genuinely do not mind which is pressed. It does
not trap focus, does not block the map, and appears
`CONSENT_BANNER_DELAY_MS` after load so the first moment on the page is a map.

Shown to everyone, with no geo-lookup and no third-party consent platform. The
decision lives in `localStorage` and the banner never returns on its own; a
**Privacy** control in the panel footer brings it back, because withdrawal has
to be as easy as granting.

#### Events

A closed union in [`src/analytics/events.ts`](src/analytics/events.ts); call
sites cannot pass free-form parameters. **No coordinates, addresses or search
...
```
src/
  analytics/  Event vocabulary, consent gate, URL sanitiser, Google tag
